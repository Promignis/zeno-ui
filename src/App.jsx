...
let DELIM_HASHTAG : string = "#";
let DELIM_MENTION : string = "@";
...
type Link = {
    delimiter : DELIM_HASHTAG | DELIM_MENTION,
    value : string,
    instances : Array <CursorPos>
}
...
class App extends React.Component<Props, State> {
  ...
  handleChange = text =>  {
    ...
    let newLinks = linkMatches.filter(
      link =>
        !this.state.links.find(
          currLink =>
            currLink.delimiter == link.delimiter && currLink.value == link.value
        )
    );
    let existingLinks = linkMatches.filter(link =>
      this.state.links.find(
        currLink =>
          ...
          currLink.delimiter == link.value
      )
    );

    this.setState({
      notes: this.state.notes.map(
        (note, index) => (index == this.state.currentNoteIndex ? text : note)
      ),
      links: this.state.links
        .map(link => {
          const found = existingLinks.filter(
            currLink =>
              currLink.delimiter == link.delimiter &&
              currLink.delimiter == link.value
          );
          const concatedInstances = found.reduce(
            (pre, foundLink) => pre.concat(foundLink.instances),
            []
          );
    ...
    return (
      ...
      >
        ...
        <Editor
          ...
          text={
            this.state.notes.find(note => note.id == this.state.currentNoteId)
              .text || ""
          }
